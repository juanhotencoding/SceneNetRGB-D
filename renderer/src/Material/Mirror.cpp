...
Mirror::Mirror(const Vector3 & Kr, int class_id, int instance_id)
...
optix::Material Mirror::getOptixMaterial(optix::Context & context)
{
    ...
    {
        optix::Program photonProgram = context->createProgramFromPTXFile(ptxpath() +  "Mirror.cu.ptx", "closestHitPhoton");
        optix::Program radianceProgram = context->createProgramFromPTXFile(ptxpath() +  "Mirror.cu.ptx", "closestHitRadiance");
