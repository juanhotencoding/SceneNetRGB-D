...
static __device__ __inline__ optix::float3 sampleUnitHemisphereCosExp(const optix::float3 & normal, const optix::float2& sample, const float exp)
{
    using namespace optix;

    float theta = acosf(powf(sample.x,1.f/exp));
...
static __device__ __inline__ optix::float3 sampleUnitHemisphereCos(const optix::float3 & normal, const optix::float2& sample)
{
    ...
    float theta = acosf(sqrtf(0.95f - sample.x));
    float phi = 2.0f * M_PIf *sample.y;
    float xs = sinf(theta) * cosf(phi);
    float ys = cosf(theta);
    float zs = sinf(theta) * sinf(phi);
