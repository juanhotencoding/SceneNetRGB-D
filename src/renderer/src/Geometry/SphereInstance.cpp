...
optix::Geometry SphereInstance::getOptixGeometry( optix::Context & context)
{
    ...
    {
        m_programBoundingBox = context->createProgramFromPTXFile(ptxpath() + "Sphere.cu.ptx", "bounds");
        m_programIntersection = context->createProgramFromPTXFile(ptxpath() + "Sphere.cu.ptx", "intersect");
