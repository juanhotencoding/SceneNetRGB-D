...
Glass::Glass( float indexOfRefraction, const Vector3 & Ks,int class_id,int instance_id)
...
optix::Material Glass::getOptixMaterial(optix::Context & context)
{
    if(!m_optixMaterialIsCreated)
    {
        ...
        optix::Program radianceClosestProgram = context->createProgramFromPTXFile(ptxpath() +  "Glass.cu.ptx", "closestHitRadiance");
        optix::Program radianceAnyHitProgram = context->createProgramFromPTXFile(ptxpath() +  "Glass.cu.ptx", "anyHitRadiance");
        optix::Program photonClosestProgram = context->createProgramFromPTXFile(ptxpath() +  "Glass.cu.ptx", "closestHitPhoton");
        optix::Program photonAnyHitProgram = context->createProgramFromPTXFile(ptxpath() +  "Glass.cu.ptx", "anyHitPhoton");
