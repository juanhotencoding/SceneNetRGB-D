...
ParticipatingMedium::ParticipatingMedium(float sigma_s, float sigma_a, int class_id, int instance_id)
    : Material(class_id,instance_id), m_sigma_a(sigma_a), m_sigma_s(sigma_s)
{
}
...
optix::Material ParticipatingMedium::getOptixMaterial(optix::Context & context)
{
    ...
    {
        optix::Program radianceProgram = context->createProgramFromPTXFile(ptxpath() +  "ParticipatingMedium.cu.ptx", "closestHitRadiance");
        optix::Program photonProgram = context->createProgramFromPTXFile(ptxpath() +  "ParticipatingMedium.cu.ptx", "closestHitPhoton");
