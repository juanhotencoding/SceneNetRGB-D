...
DiffuseEmitter::DiffuseEmitter(const Vector3 & power, const Vector3 & Kd, int class_id, int instance_id)
...
optix::Material DiffuseEmitter::getOptixMaterial(optix::Context & context)
{
    ...
    {
        optix::Program radianceProgram = context->createProgramFromPTXFile(ptxpath() + "DiffuseEmitter.cu.ptx", "closestHitRadiance");
        ...
        m_optixMaterial->setClosestHitProgram(RayType::PHOTON, context->createProgramFromPTXFile(ptxpath() +  "DiffuseEmitter.cu.ptx", "closestHitPhoton") );
        m_optixMaterial->setAnyHitProgram(RayType::SHADOW, context->createProgramFromPTXFile(ptxpath() + "DiffuseEmitter.cu.ptx", "gatherAnyHitOnEmitter"));
